    ...
    def post(self, request):
        try:
            if request.data['title'] == "":
                request.data['title'] = request.user.nickname + "의 글"

            get_object_or_404(BoardModel, user_id= request.user.id, title= request.data['title'],
...
class QuestionDetailAPIView(APIView):
    ...
    @login_check
    #질문 글 수정 API(title, field) -> 질문자만 수정 가능
    def put(self, request, pk):
        try:
            # pk와 id가 동일한 객체를 가져옴
            ...
            if 'title' not in request.data:
                request.data['title'] = board_obj.title
            elif request.data['title'] == "":
                request.data['title'] = request.user.nickname + "의 글"

            if 'field' not in request.data:
                request.data['field'] = board_obj.field


            if BoardModel.objects.exclude(pk=pk).filter(user_id = request.user.id,title=request.data['title'],
                                                            field=request.data['field']).exists():
            ...
            if board_id == request.user.id:
                serializer = BoardSerializer(board_obj, data=request.data, partial=True)
